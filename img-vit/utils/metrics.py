...
def make_evaluate(s_s_references_list, s_s_hypothesis_list, tokenizer):
    ...
    for ref in s_s_references_list:
        temp = []
        for r in ref:
            temp.append([word2idx.get(i,unk_idx) for i in r])
        t_references_list.append(temp)
        
    
    for hyp in s_s_hypothesis_list:
        t_hypothesis_list.append(word2idx.get(h,unk_idx) for h in hyp)
